...
const PendingApplications = () => {
	...
	useEffect(() => {
		if (data?.data) {
			const pendingApplications = data.data.filter(
				(item: ReturnType) => item.status === "queue",
			);
			setSearchValues(pendingApplications);
		}
	}, [data]);
	...
	const handleSearchBar = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		const filteredData = data?.data
			.filter((item: ReturnType | null) => item?.status === "queue")
	...
	return (
		<div>
			<ActionContainer title="Pending Applications" onChange={handleSearchBar}>
				<>
					{isLoading ? (
						<IsLoadingData />
					) : (
...
export default withQueryClient(PendingApplications);

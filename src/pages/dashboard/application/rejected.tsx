...
const RejectedApplication = () => {
	...
	useEffect(() => {
		if (data?.data) {
			const rejectedApplications = data.data.filter(
				(item: ReturnType) => item.status === "rejected",
			);
			setSearchValues(rejectedApplications);
		}
	}, [data]);
	...
	const handleSearchBar = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		const filteredData = data?.data
			.filter((item: ReturnType) => item.status === "rejected")
			.filter((item: ReturnType) => {
				// Then, apply the search criteria
				return (
	...
	return (
		<div>
			<ActionContainer title="Rejected Applications" onChange={handleSearchBar}>
...
export default withQueryClient(RejectedApplication);
